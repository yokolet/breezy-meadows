...
class SignupHandler(utils.Handler):
    ...
    def post(self):
        ...
        if have_error:
            self.render("signup.html", on_signup="on_singup",
    ...
    def register(self, values):
        ...
        if user:
            ...
            self.render("signup.html", on_signup="on_signup",
                        errors=errors, values=values)
        else:
            user = models.User.register(values['username'],
                                        values['password'],
                                        values['email'])
            user.put()
...
class LoginHandler(utils.Handler):
    ...
    When a user is already logged-in, the page will be redirected to the
    welcome page. Otherwise, it shows a login form. Parameters in the
    input fields are validated. If the input parameters are matched to
    a redigered user, the welcome pages will show up. If there's an error,
    ...
    def get(self):
        ...
        if user:
            ...
            self.render('login.html', on_login="on_login",
                        errors={}, values={})
